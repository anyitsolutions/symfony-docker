...
class SkillRepository extends ServiceEntityRepository implements SkillRepositoryInterface
{
    ...
    public function findByFilter(SkillsFilter $filter): PaginationResult
    {
        ...
        if (is_array($filter->includedSkillIds)) {
            $qb->andWhere('s.id IN (:includedSkillIds)')
                ->setParameter('includedSkillIds', $filter->includedSkillIds);
        }

        $qb->leftJoin('s.skillGroup', 'sg');
        $qb->andWhere('sg.hidden = false');
        ...
        return new PaginationResult(
            ...
            Pager::fromPager($filter->pager, $paginator->count())
        );
    }
